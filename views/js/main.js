...
var resizePizzas = function(size) {
  ...
  function changeSliderLabel(size) {
    var PizzaSize = document.getElementById("pizzaSize").innerHTML; //  sets euch pizzaSizes
    switch(size) {
      case "1":
      PizzaSize  = "<p>Small</p>"; // added the p element in the DOM
        return;
      case "2":
        PizzaSize = "<p>Medium</p>";// added the p element in the DOM
        return;
      case "3":
        PizzaSize = "<p>Large</p>";// added the p element in the DOM
        return;
      default:
  ...
  function determineDx (elem, size) {
    var oldWidth = elem.offsetWidth,
     ...
     ReturnOLWi = (sizeSwitcher(size) - oldWidth / windowWidth) * windowWidth; // multi and add of offsetwidth randomPizzas
   return ReturnOLWi;
  ...
  function changePizzaSizes(size) {
    var randomPizza = document.getElementsByClassName("randomPizzaContainer"), // vars getting randomPizzaContainer Classes
    random = document.querySelector(".randomPizzaContainer"),
     length = randomPizza.length,
     dx = determineDx(random, size),
     newwidth = random.offsetWidth + dx + 'px';
...
var pizzasDiv = document.getElementById("randomPizzas"); // the variable for pizzasDiv element
for (var i = 2; i < 100; i++) {
  pizzasDiv.appendChild(pizzaElementGenerator(i)); // adding elements into DOM
}
...
function logAverageFrame(times) {   // times is the array of User Timing measurements from updatePositions()
  var numberOfEntries = times.length,
   sum = 0;
  for (var i = numberOfEntries-1; i > numberOfEntries -2; i--) {
    sum +=  times[i].duration; // multi the times element to duration
  }
...
// for loop while scrolling the dom to animated the pizzas element
...
document.addEventListener('DOMContentLoaded', function() {
  ...
  for (var i = 0; i < 24; i++) { // for loop to create img pizza
